...
<div>
    <div class="opt">
        <a-form :model="form" layout="inline" ref="forms" :locale="locale">
            <a-form-item label="电表编号" name="meaternum">
                <a-input v-model="form.meaternum" placeholder="请输入电表编号" />
            </a-form-item>
            <a-form-item label="制造商" name="manufacturer">
                <a-input v-model="form.manufacturer" placeholder="请输入制造商" />
            </a-form-item>
            <a-form-item label="电表类型" name="meatertype">
                <a-select v-model="form.meatertype" placeholder="请选择电表类型" style="width: 174px">
                    <a-select-option v-for="item in meterType" :value="item.id" :key="item.name">
                        {{ item.name }}
                    </a-select-option>
                </a-select>
            </a-form-item>
    ...
    <a-table :columns="columns" :data-source="recordData" bordered :pagination="pagination" @change="handleTableChange" :scroll="{ y: 600 }" :rowKey="columnId" :loading="tableLoading">
